...
/// Blocking SD card API mode and Delay
pub struct Blocking;
impl Mode for Blocking {}

/// async/await SD card API mode and Delay
pub struct Async;
impl Mode for Async {}

#[allow(private_bounds)]
/// Async or Blocking mode of the APIs
pub trait Mode: sealed::SealedMode {}

mod sealed {
    impl SealedMode for super::Blocking {}
    impl SealedMode for super::Async {}
    pub(super) trait SealedMode {}
}
...
///
/// [`SpiDevice`]: embedded_hal::spi::SpiDevice
pub struct SdCard<SPI, CS, DELAYER, M: Mode> {
    inner: RefCell<SdCardInner<SPI, CS, DELAYER, M>>,
}

impl<SPI, CS, DELAYER, M> SdCard<SPI, CS, DELAYER, M>
where
    M: Mode,
{
    ...
    pub fn new_with_options(
        ...
        delayer: DELAYER,
        options: AcquireOpts,
    ) -> SdCard<SPI, CS, DELAYER, M> {
        SdCard {
            inner: RefCell::new(SdCardInner::<_, _, _, M> {
                spi,
                cs,
                delayer,
                card_type: None,
...
impl<SPI, CS, DELAYER> SdCard<SPI, CS, DELAYER, Blocking>
where
    ...
    DELAYER: embedded_hal::delay::DelayNs,
{
    ...
    pub fn new(spi: SPI, cs: CS, delayer: DELAYER) -> SdCard<SPI, CS, DELAYER, Blocking>
    where
        SPI: embedded_hal::spi::SpiDevice<u8>,
        CS: embedded_hal::digital::OutputPin,
        DELAYER: embedded_hal::delay::DelayNs,
    {
        SdCard::new_with_options(spi, cs, delayer, AcquireOpts::default())
    }
...
impl<SPI, CS, DELAYER> SdCard<SPI, CS, DELAYER, Async>
where
    SPI: embedded_hal_async::spi::SpiDevice<u8>,
    CS: embedded_hal::digital::OutputPin,
    DELAYER: embedded_hal_async::delay::DelayNs,
{
    /// Create an async SDCard API
    #[cfg(feature = "asynch")]
    pub fn new_async(spi: SPI, cs: CS, delayer: DELAYER) -> SdCard<SPI, CS, DELAYER, Async>
    where
        SPI: embedded_hal_async::spi::SpiDevice<u8>,
        CS: embedded_hal::digital::OutputPin,
        DELAYER: embedded_hal_async::delay::DelayNs,
    ...
    /// Useful if you need to re-clock the SPI, but does not perform card
    /// initialisation.
    // TODO: Without nightly features for Async closures we cannot get a mut SPI and pass it to a custom function.
    pub async fn spi<'a, T, F>(&'a self, mut async_func: F) -> T
    where
        F: async_fn_traits::AsyncFnMut1<&'a mut SPI, Output = T>,
    {
        let mut inner = self.inner.borrow_mut();
        async_func(&mut inner.spi).await
    }

    // pub async fn spi_mut(&self) -> &mut SPI {
    //     let mut inner = self.inner.borrow_mut();
    //     inner
    // }
...
impl<SPI, CS, DELAYER> BlockDevice for SdCard<SPI, CS, DELAYER, Blocking>
where
    SPI: embedded_hal::spi::SpiDevice<u8>,
    CS: embedded_hal::digital::OutputPin,
    DELAYER: embedded_hal::delay::DelayNs,
...
#[cfg(feature = "asynch")]
impl<SPI, CS, DELAYER> crate::blockdevice::AsyncBlockDevice for SdCard<SPI, CS, DELAYER, Async>
where
    SPI: embedded_hal_async::spi::SpiDevice<u8>,
    CS: embedded_hal::digital::OutputPin,
    DELAYER: embedded_hal_async::delay::DelayNs,
...
struct SdCardInner<SPI, CS, DELAYER, M: Mode> {
    spi: SPI,
    cs: CS,
    delayer: DELAYER,
...
impl<SPI, CS, DELAYER> SdCardInner<SPI, CS, DELAYER, Blocking>
where
    SPI: embedded_hal::spi::SpiDevice<u8>,
    CS: embedded_hal::digital::OutputPin,
    DELAYER: embedded_hal::delay::DelayNs,
...
#[cfg(feature = "asynch")]
impl<SPI, CS, DELAYER> SdCardInner<SPI, CS, DELAYER, Async>
where
    SPI: embedded_hal_async::spi::SpiDevice<u8>,
    CS: embedded_hal::digital::OutputPin,
    DELAYER: embedded_hal_async::delay::DelayNs,
{
        ...
        self.with_chip_select(async {
            if blocks.len() == 1 {
                ...
                self.write_data(DATA_START_BLOCK, &blocks[0].contents)
                    .await?;
                self.wait_not_busy(Delay::<Async>::new_write()).await?;
    ...
    async fn read_data(&mut self, buffer: &mut [u8]) -> Result<(), Error> {
        // Get first non-FF byte.
        let mut delay = Delay::<Async>::new_read();
        let status = loop {
            ...
            delay
                .delay_async(&mut self.delayer, Error::TimeoutReadBuffer)
                .await?;
        };
        if status != DATA_START_BLOCK {
            return Err(Error::ReadError);
    ...
    async fn acquire(&mut self) -> Result<(), Error> {
        ...
        let future = async {
            ...
            // Check card version
            let mut delay = Delay::<Async>::new_command();
            let arg = loop {
                ...
                delay
                    .delay_async(&mut self.delayer, Error::TimeoutCommand(CMD8))
                    .await?;
            };
            ...
            while self.card_acmd(ACMD41, arg).await? != R1_READY_STATE {
                delay
                    .delay_async(&mut self.delayer, Error::TimeoutACommand(ACMD41))
                    .await?;
            }
    ...
    ///
    /// Always releases the chipselect, even if the function errors.
    async fn with_chip_select<F, T>(&mut self, fut: F) -> Result<T, Error>
    where
        F: AsyncFnMut<&mut Self, Output = Result<T, Error>>,
    {
        self.cs_low()?;
        let result = fut(&mut self).await;
        self.cs_high()?;
        result
    }
    ...
    async fn card_command(&mut self, command: u8, arg: u32) -> Result<u8, Error> {
        if command != CMD0 && command != CMD12 {
            self.wait_not_busy(Delay::<Async>::new_command()).await?;
        }
        ...
        loop {
            ...
            delay
                .delay_async(&mut self.delayer, Error::TimeoutCommand(command))
                .await?;
        }
    }
    ...
    /// Spin until the card returns 0xFF, or we spin too many times and
    /// timeout.
    async fn wait_not_busy(&mut self, mut delay: Delay<Async>) -> Result<(), Error> {
        loop {
            let s = self.read_byte().await?;
            if s == 0xFF {
                break;
    ...
    M: Mode,
{
    ...
    fn new(max_retries: u32) -> Delay<M> {
        Delay {
...
impl Delay<Async> {
    ...
    /// value `err` is returned. Otherwise we wait for 10us and then return
    /// `Ok(())`.
    async fn delay_async<T>(&mut self, delayer: &mut T, err: Error) -> Result<(), Error>
    where
...
mod test {
    ...
    #[test]
    fn test_spi_async_fn() {
        let mut spi = SpiMock::new(&[SpiTransaction::write(0x09)]);
        let mut cs = PinMock::new(&[PinTransaction::get(PinState::Low)]);
        let delayer = NoopDelay::new();
        let mut sd_card = SdCard::new_async(spi, cs, delayer);

        sd_card.spi(mock_spi_write(spi));
    }

    pub fn mock_spi_write<F>(spi: &mut SpiMock<SpiTransaction<u8>>) -> F
    where
        F: AsyncFnMut1<(), Output = Result<(), embedded_hal_async::spi::Error>>,
    {
        async {
            spi.write([0x09]).await?;
            Ok(())
        }
    }
}
